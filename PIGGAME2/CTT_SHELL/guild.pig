...
--PROCESS t_juntuan
STOCK_A1 = FOREACH STOCK_A GENERATE id,str_name,( (dt_create IS NOT NULL and dt_create != '') ? dt_create : ToString(CurrentTime(),'yyyy-MM-dd HH:mm:ss') ) as dt_create:chararray ,chuang_jian_zhe,ToString(CurrentTime(),'yyyy-MM-dd HH:mm:ss') as date:chararray ; 
...
--INFOMATION NEED
STOCK_FINAL = FOREACH STOCK_A_B_C_D_G GENERATE $4 as date,$4 as date_modify,$gameid as game_id,$serverid as server_id,CONCAT((chararray)'$ALIAS',CONCAT('_',CONCAT((chararray)$serverid,CONCAT('_',(chararray)$0)))) as game_guild_id,CONCAT((chararray)'$ALIAS',CONCAT('_',CONCAT((chararray)$serverid,CONCAT('_',(chararray)$1)))) as game_guild_name,$2 as game_guild_create_date,(chararray)$5 as game_guild_leader_name,(chararray)$15 as msi_leader,$17 as mobo_id,(chararray)$19 as fullname,$15 as accid_leader ,(chararray)$3 as role_id_leader;
