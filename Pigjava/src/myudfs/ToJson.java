...
public class ToJson extends EvalFunc<String> {
	...
	public String exec(Tuple input) throws IOException {
		...
		if (myProperties == null) {
			...
			myProperties = UDFContext.getUDFContext().getUDFProperties(
					this.getClass());
		}
	...
	protected static JSONObject tupleToJson(Tuple t,
			FieldSchema tupleFieldSchema) throws ExecException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static JSONArray bagToJson(DataBag bag, FieldSchema bagFieldSchema)
	 ...
	 */
	private static Object fieldToJson(Object value, FieldSchema fieldSchema)
			throws ExecException {
		...
		case DataType.MAP:
			throw new ExecException(
					"Map type is not current supported with JsonStorage");

		case DataType.BYTE:
			throw new ExecException(
					"Byte type is not current supported with JsonStorage");

		case DataType.BYTEARRAY:
			throw new ExecException(
					"ByteArray type is not current supported with JsonStorage");

		default:
			throw new ExecException(
					"Unknown type is not current supported with JsonStorage");
		}
	}
