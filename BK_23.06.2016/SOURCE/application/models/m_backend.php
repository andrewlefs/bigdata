 ...
 * @author tailm
 */
class M_backend extends MeAPI_Model {
	...
	$rs = $this->db_slave->update($table,$param);// echo $this->db_slave->last_query();
        return $rs;
    }
    ...
    function jqxGetAppName($name) {
        $query = $this->db_slave->where('name_app', $name)->get($this->_table)->row_array();
        return $query;
    }
...
//<<<<<<< HEAD
    ...
    function get_app_add_tab(){
        $sql = $this->db_slave->select('id_app, name_app')
	    	              ->from('app')
			      ->get();
        if (is_object($sql)) {
            return $sql->result_array();
