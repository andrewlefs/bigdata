...
STOCK_B2 = FOREACH STOCK_B1 GENERATE myudfs.ToJson(myudfs.TestHex(roleguid)) as roleguid,myudfs.ToJson(myudfs.TestHex(societyguid)) as societyguid,( (jointime IS NOT NULL and jointime != '') ? jointime : ToString(CurrentTime(),'yyyy-MM-dd HH:mm:ss') ) as jointime:chararray; 
