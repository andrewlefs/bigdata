...
--JOIN STOCK_B_D_A & account_service
--STOCK_B_D_A_G = JOIN STOCK_B_D_A BY $10 LEFT OUTER, STOCK_G BY mobo_service_id;

--INFOMATION NEED
STOCK_FINAL = FOREACH STOCK_B_D_A GENERATE $14 as date,$14 as date_modify,$gameid as game_id,$serverid as server_id,CONCAT((chararray)$gameid,CONCAT('_',CONCAT((chararray)$serverid,CONCAT('_',(chararray)$0)))) as game_guild_id,CONCAT((chararray)$gameid,CONCAT('_',CONCAT((chararray)$serverid,CONCAT('_',(chararray)$2)))) as game_guild_name,myudfs.TimestamptoDate($13) as game_guild_create_date,$5 as game_guild_leader_name,$10 as msi_leader,$10 as accid_leader ,$4 as role_id_leader,$14 as date_modify_new;

--STORE STOCK_FINAL INTO 'output/test113';
